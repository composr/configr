...
public class AclValidator {
    ...
    public AclValidator(@Autowired HttpServletRequest request, @Autowired MetadataService metadataService){
    ...
    private boolean validateRoles(Action action, Set<ACL> acls, Collection<GrantedAuthority> authorities) {        
        for (ACL acl : acls ) {
    ...
    private boolean roleInToken(ACL acl, Collection<GrantedAuthority> authorities) {
        ...
        return authorities.stream().anyMatch(grantedAuthority -> grantedAuthority.getAuthority().matches(".*?" + role + ".*?"));
    }
}
